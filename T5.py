...
def T5():
    ...
    def astar_est_time(start_node, end_node, start_time, curr_best) -> float:
        ...
        while priority_queue:
            ...
            if min_time_to_node[current_node] > curr_best:
                return float('inf')
        ...
        while len(available_drivers) > 0:
            if len(available_passengers) > 0:
                ...
                if not driver:
                    ...
                    pickup_point = search(tree_root, passenger.start_location)

                    index = -1
                    driver_node = search(avail_drivers_tree, passenger.start_location)
                    driver_coords = driver_node.coords
                    driver = driver_node.obj
                    if driver not in available_drivers:
                        print("WHAT THE FUCK")
                        exit()
                    end_node = search(tree_root, passenger.start_location)

                    #where to drop-off passenger
                    drop_off_node = search(tree_root, passenger.desired_location)
                    ...
                    drop_off_time = astar_est_time(end_node, drop_off_node, driver.start_time, float('inf') )
                    profit = drop_off_time - min_pickup_time

                    for i in range(len(available_drivers)):

                        # if checking the closest driver again, quit early - already been checked
                        if available_drivers[i] == driver:
                            continue
                        ...
                        timeDrive = astar_est_time(start_node, end_node, available_drivers[i].start_time, min_pickup_time)
                        timeDropOff = astar_est_time(end_node, drop_off_node, available_drivers[i].start_time, float('inf') )
                        driveprofit = timeDropOff - timeDrive
                        # if it is less than the previous min pickup time, replace the best driver
                        # more efficient driver/passenger pairing found!

                        #AS LONG AS driver is making some money
                        if (timeDrive < min_pickup_time and driveprofit>0) or (timeDrive < 1.25*min_pickup_time and driveprofit>0):

                            min_pickup_time = timeDrive
                            drop_off_time = timeDropOff
                            profit = driveprofit
                ...
                if driver.drive_passenger_T3(passenger, min_pickup_time,
                                            drop_off_time, current_time = curr_time) == -1:  # CHANGE/ CREATE METHOD TO BE T3 alg instead of haversine_distance
...
if __name__ == "__main__":
    from benchmarking import run_t5
